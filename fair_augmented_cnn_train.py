...
def main():
    ...
    PATIENCE = 4
    TARGET_SIZE = (224, 224)
    DATASET_ROOT = os.path.join(workspace_dir, "augmented_dataset")
    ...
    assert len(train_ds) == 1459, f"Expected 1459 train samples, got {len(train_ds)}"
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        if epoch_val_loss < best_val_loss:
            ...
            best_epoch = epoch
            patience_counter = 0
            model_save_path = os.path.join(models_dir, "fair_augmented_cnn.pth")
    ...
    metrics_save_path = os.path.join(results_dir, "fair_augmented_cnn_metrics.json")
