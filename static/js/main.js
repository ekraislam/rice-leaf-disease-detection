/* ── Full i18n Translation Dictionary ── */
        ...
        async function submitBatchAnalysis() {
            ...
            if (loadingState) {
                ...
                if (stepText) stepText.textContent = currentLang === 'bn' ? `মাঠের ${window.batchSelectedFiles.length}টি নমুনা প্যারালাল প্রসেসিং হচ্ছে…` : `Processing ${window.batchSelectedFiles.length} field samples in parallel…`;
            }

            const formData = new FormData();
            window.batchSelectedFiles.forEach(f => {
            ...
            try {
                ...
                try {
                    ...
                    if (!response.ok) {
                        throw new Error(currentLang === 'bn' 
                            ? `সার্ভার আপডেট হয়নি (HTTP ${response.status})। অনুগ্রহ করে আপনার টার্মিনালে 'python app.py' বন্ধ করে পুনরায় চালু (Restart) করুন।` 
                            : `Server returned HTTP ${response.status}. Please restart 'python app.py' in your terminal.`);
                    }
                    throw parseErr;
                }
                ...
                if (loadingState) loadingState.style.display = 'none';
                if (emptyState) emptyState.style.display = 'flex';
                alert((currentLang === 'bn' ? 'মাঠ নিরীক্ষা ব্যর্থ হয়েছে: ' : 'Field audit failed: ') + err.message);
            } finally {
